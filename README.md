...
### Hybrid Devices (4 devices)

| Device | Telemetry | Commands | Description |
|--------|-----------|----------|-------------|
| mobile_light | energy_kwh, on, level | set(on: bool, level: str) | Smart light 
...
python -m src.devices.door_window --device-id window_1
...
python -m src.devices.actuators --device sprinkler --device-id sprinkler

# Hybrid Devices
python -m src.devices.actuators --device mobile_light --device-id mobile_light
python -m src.devices.utility_meter --meter gas --device-id gas_meter
python -m src.devices.utility_meter --meter electricity --device-id electricity_meter
python -m src.devices.utility_meter --meter water --device-id water_meter
```
...
├── presentation/
│   └── Smart_Home_Safety_System_Presentation.pdf
└── src/
    ├── __init__.py
...
**Alarm Switch Telemetry Pattern:**
- Alarm switch publishes armed state as telemetry (not just accepting commands)
- Enables bidirectional control (manual toggle + remote command)
- Manager updates global `armed` state from alarm_switch telemetry
- Demonstrates realistic IoT pattern where devices have both local and remote control
